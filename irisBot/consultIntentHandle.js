...
module.exports = function (intentRequest) {
  ...
  // for getting valuee of email in slack
  ...
  if (source === 'DialogCodeHook') {
    //check if someone type an uttereance of book a consulatation  in project description
    if (config.current_step == 'discussIntent') {
      ...
      if (intentRequest.requestAttributes != null) {
        //intentRequest.requestAttributes is null for web
        var platform =
          intentRequest.requestAttributes['x-amz-lex:channel-type'];
      } else {
        var platform = 'Web';
      ...
      // check for html rendering on different platforms
      var message =
        'I have booked your free 30 minutes consultation with our expert. We will call between ' +
        config.user_details.user_time +
        ' (IST, + 5.5 GMT) on ' +
      ...
      var status = common.sendInvite(
        '<span>Hi ' + query_form.user_name + ', <br> This is to inform you that, your appointment for free consultation session has been confirmed. You will receive a phone call from us on ' + query_form.user_day + ' between ' + query_form.user_time + ' (IST, + 5.5 GMT). <br><br>If you have any query, please feel free to contact us at 0172-55316 or you can write us at <a href="mailto:example@email.com">info@prologictechnologie.in</a>  <br><br> Thanks & Regards<br>Prologic Technologies</span> ', query_form.user_email, 'Consultation Appointment with Prologic Technologies'
      );
      sendSlackMsg(slack_msg, myResult => {
        // console.log ('Slack message sent : ' + myResult);
        var status = common.sendEmail(
